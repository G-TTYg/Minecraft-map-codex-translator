...
2. Choose one user-facing export mode and explain it clearly.
...
2. Scan and classify text.
   ...
   - Treat entity `CustomName` as selector identity when any command or JSON selector uses `@e[name=Guide]`, negated `@e[name=!Guide]`, or `@e[nbt={CustomName:...}]`. Review `selector_identity.json`. The scanner links exact static names across command blocks and datapack functions, marks selector-literal and matching `entity_name` rows as `selector_identity_coupled`, and removes copied-world patch modes from them. Never translate `tag`, `type`, `scores`, `predicate`, or other selector logic fields.
   - Preserve selector-coupled `CustomName` source text with `review_status: intentional_name` and a concrete selector reason. If visible NPC names must be localized, first implement and validate an exact map-logic migration from name/NBT matching to stable entity tags; do not merely translate both strings or use global replacement. Dynamic macro selector names and unmatched references remain explicit review/runtime-test items.
...
4. Translate like a localization editor.
   ...
   - Do not translate an entity name or selector predicate marked `selector_identity_coupled`. Keep the unit and all segments source-equal, classify them as `intentional_name`, and explain which `@e` reference requires preservation.
