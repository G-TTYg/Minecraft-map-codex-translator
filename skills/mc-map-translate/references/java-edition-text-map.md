...
- Commands: `tellraw`, `title`, `bossbar`, `scoreboard`, `team`, `summon`, `data merge`, `item`, and `loot` frequently embed JSON text components or SNBT containing text components.
- Datapack JSON: advancements, predicates, item modifiers, loot tables, recipes, and custom data may contain text components.
- NBT world data: command blocks, signs, books, entities, item display names/lore, scoreboards, teams, storage, and map data can contain player-facing strings.
